...
@implementation EmulatorsDisableController

- (id)init;
{
	if (DEBUG_MODE) NSLog(@"EmulatorsDisableController - init");
	self = [super init];
	if (self == nil) return nil;
	
	[self addLabel:@"com.bgan1982.Emulators.EmulatorsDisableController"];
	[self setListTitle:@"Enable/Disable Emulators"];
...
- (void)dealloc
{
	if (DEBUG_MODE) NSLog(@"EmulatorsDisableController - dealloc");
...
{
	if (DEBUG_MODE) NSLog([NSString stringWithFormat:@"EmulatorsDisableController - itemSelected, row=%i",fp8]);
	switch (fp8)
	{
		case 0:
			break;
